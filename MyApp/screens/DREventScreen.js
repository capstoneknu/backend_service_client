...
const DREventScreen = () => {
  const {events, history, fetchEvents, fetchHistory, participateEvent,
    toggleNotification, simulateSaving, isLoading} = useDRStore();
  ...
  // 참여 중인 이벤트가 있으면 주기적 갱신
  useEffect(() => {
    const hasParticipating = events.some(e => e.isParticipating && e.status === 'active');
    if (hasParticipating) {
      const interval = simulateSaving();
      return () => clearInterval(interval);
    }
  }, [events.map(e => e.isParticipating).join(',')]);
